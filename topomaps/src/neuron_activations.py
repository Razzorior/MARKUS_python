from topomaps.src.models import train_or_load_model, load_data
import numpy as np
...
def get_NWPs(model, data, layers, n_random_examples, error_mode=None):
    ...
    NWPs = {'values': dict(), 'error_mode': error_mode}

    for layer in layers:
        print("Calculating NWPs of layer {}".format(layer))
        activations = get_activations(extractor, layer, example_inputs)
        layer_NWPs = compute_NAPs(activations)
        NWPs['values'][layer] = layer_NWPs
